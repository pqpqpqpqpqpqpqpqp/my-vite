        ...
        <div className="h-screen flex flex-col">
            ...
            <main className="flex flex-1 overflow-hidden">
                <aside className="w-90 bg-gray-100 p-6 overflow-y-auto">
                    <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-2">여행 일정</h2>
                    <div className="mb-6">
                    ...
                    <div className="flex space-x-2 mb-4">
                        {[...tripGroupByOrder.keys()].map(day => (
                            <button
                                key={day}
                                className={`px-3 py-1 rounded ${selectedDay === day ? 'bg-blue-600 text-white' : 'bg-gray-300'}`}
                    ...
                    <div>
                        {tripGroupByOrder.get(selectedDay)?.map((place) => (
                            <div key={place.orderInDay} className="mb-4">
                                <div className="text-lg font-semibold">{place.placeName}</div>
                                <div className="text-sm text-gray-600">{place.visitTime}</div>
                                <div className="text-gray-700">{place.placeMemo}</div>
                            </div>
                        ))}
                    </div>
                </aside>
